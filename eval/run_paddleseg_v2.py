"""
PaddleSeg v2 — PP-LiteSeg with Focal Loss + class weights.
Fixes the class imbalance problem from v1 (all-background prediction).
"""
import os
import json
import time
...
RESULTS_DIR = _RESULTS_BASE / "paddleseg_v2"
...
def compute_class_weights(split_dir, src_dir):
    ...
    for line in lines[:50]:  # Sample subset for speed
        parts = line.split("\t")
    ...
    # Cap background weight, boost defect weights
...
def main():
    ...
    print("=== PaddleSeg v2: Focal Loss + Dice + Class Weights ===\n")
    ...
    combined_loss = WeightedCELoss(weight=class_weights)
    ...
    print(f"\nTraining {EPOCHS} epochs, {iters} iters (Weighted CE, CPU)...")
    start = time.time()
    ...
    print("\n" + "=" * 50)
    print("RESULTS — PaddleSeg v2 (Weighted CE, 50 epochs)")
    print("=" * 50)
