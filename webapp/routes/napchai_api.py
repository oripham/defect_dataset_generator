"""
webapp/routes/napchai_api.py — Napchai (Nắp Chai) Defect API
=============================================================

Endpoints:
  POST /api/napchai/preview            → generate 1 image, return immediately
  POST /api/napchai/batch              → start batch job (background thread)
  GET  /api/napchai/batch/<job_id>     → poll batch progress
  GET  /api/napchai/ok-images          → list OK images for a defect class
  POST /api/napchai/save               → save result to disk
  GET  /api/napchai/results            → list saved result files

Auto-circle-detect via Hough — no user-drawn mask required.
CV-only synthesis — no GPU needed.
"""
...
napchai_bp = Blueprint("napchai_api", __name__)
...
# ── API: product / defect catalogue ───────────────────────────────────────────

@napchai_bp.get("/api/napchai/products")
def api_products():
    out = {}
...
@napchai_bp.get("/api/napchai/ok-images")
def api_ok_images():
...
@napchai_bp.post("/api/napchai/preview")
def api_napchai_preview():
    """
    Generate 1 Napchai defect image.

    Body:
    {
      "image_b64":    str,           # base64 PNG OK image
      "defect_type":  str,           # "scratch" | "mc_deform" | "ring_fracture" | "dent"
      "params":       {
        "seed":        int,
        "intensity":   float,
        ...defect-specific...
    ...
    result = _engine_post("/api/napchai/preview", {
        "image_b64":   img_b64,
...
@napchai_bp.post("/api/napchai/save")
def api_napchai_save():
    body        = request.get_json(force=True, silent=True) or {}
...
@napchai_bp.post("/api/napchai/batch")
def api_napchai_batch():
    body = request.get_json(force=True, silent=True) or {}
...
@napchai_bp.get("/api/napchai/batch/<job_id>")
def api_napchai_batch_status(job_id):
    with _batch_lock:
...
@napchai_bp.get("/api/napchai/results")
def api_napchai_results():
